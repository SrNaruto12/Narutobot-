   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = ' escola a resolução amiguinho (a) Cabeçudo(a), \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui estão os acordes da música *${value}* Cabeçudo(a)\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
  ...
  case 'cektanggal':
   ...
   case 'bitly':
    ...
    case 'bucin':
           ...
           map()
               .then(buffer => {
                   client.sendMessage(id, '[Espere tomando um pouco meu amiguinho (a) 🤝', MessageType.text)
                   client.sendMessage(id, buffer, MessageType.image)
               })
               .catch(err => {
                   console.log(err)
      ...
      case 'stickergif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[❗] Cabeçudo(a) o comando  *!stickerGif* Seu video tem que esta com 10 segundos no maximo :)!', id)
                )
            }
            break     
     case 'sticker':
